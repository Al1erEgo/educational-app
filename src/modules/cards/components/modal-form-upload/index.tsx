...
import { Control, Controller, ControllerRenderProps, UseFormSetError } from 'react-hook-form'
...
import { getBase64, getModalCardFormControllerName } from '@/modules/cards/utils'

type ModalFormUploadProps = {
  name: string
  control: Control
  error?: any
...
type HandleUploadChangeType = <T extends keyof (ModalCardFormDataType & ModalPackFormDataType)>(
  event: UploadChangeParam<UploadFile<any>>,
  field: ControllerRenderProps<FieldValues, T>
) => void

export const ModalFormUpload: FC<ModalFormUploadProps> = ({ name, control, error, setError }) => {
  const uploadControllerName = getModalCardFormControllerName(name, 'deck', 'Img')
  ...
  const handleUploadChange: HandleUploadChangeType = async ({ fileList: newFileList }, { name, onChange }) => {
    ...
    <Form.Item validateStatus={error ? 'error' : ''} help={error?.message}>
      ...
      <Controller
        ...
        render={({ field }) => (
          ...
          >
            {!field.value && <Button icon={<UploadOutlined />}>Upload {name} image</Button>}
          </Upload>
        )}
      />
      {preview}
