...
export const usePacksHandlers: UsePacksHandlersType = (
  ...
  const handleAddNewPack: HandleAddNewPackType = async (
    ...
    await modalHandlers.addPackModal({
      cardsPack: { name: name, private: isPrivate },
    })
  }
  ...
  const handleOk: HandleOkType = (id?, newName?, isPrivate?) => {
    if (id) {
      modalHandlers.updatePackModal({
        cardsPack: { _id: id, name: newName, private: isPrivate },
      })
    }
  }
  const handleDeleteOk = async (packsId?: string) => {
    console.log('packsId', packsId)
    await modalHandlers.deletePackModal({ id: packsId })
  }
